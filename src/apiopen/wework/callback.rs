...
async fn handle_menu_click(
    ...
    if let Some(menu_subject) = menu_config.get(menu_id) {
        // 查询daily_subject表验证科目是否有效
        let subject_db = db::DailySubject::new();
        let subject_record = match subject_db.find_subject(&menu_subject.subject, &menu_subject.grade) {
            Some(rec) => rec,
            None => {
                tracing::warn!("科目未找到或已禁用: {} {}", menu_subject.subject, menu_subject.grade);
                return Some(format!("该科目暂未开放：{}{}", menu_subject.grade, menu_subject.subject));
            }
        };

        tracing::info!("找到科目: id={}, name={}, grade={}", subject_record.idsubject, subject_record.myname, subject_record.grade);

        // 调用出题 API，传入科目ID
        return handle_daily_quiz_generate(&user.sid, &subject_record.idsubject, &menu_subject.grade, &menu_subject.subject, user.money78 as i32).await;
    }
...
/// 处理出题请求
async fn handle_daily_quiz_generate(uid: &str, idsubject: &str, grade: &str, subject_name: &str, current_score: i32) -> Option<String> {
    use crate::memcached::{get_memcached_client, QuizState};

    tracing::info!("开始出题: uid={}, idsubject={}, grade={}, subject={}", uid, idsubject, grade, subject_name);

    // 调用出题 API（传入科目名称，不是ID）
    match crate::generate_question(grade, subject_name, Some(current_score)).await {
        Ok(result) => {
            // 存储到 Memcached
            ...
            Some(format!(
                "【{}{}题】\n{}{}\n\n请直接回复答案",
                grade, subject_name, result.question, hint_text
            ))
        }
        Err(e) => {
...
async fn handle_daily_quiz_judge(wechat_userid: &str, user_answer: &str) -> Option<String> {
    ...
    use db::{DailyUserScore, DailyAnswerRecord};
    ...
    match crate::judge_answer(
        ...
        Ok(result) => {
            ...
            // 更新用户积分和能力分
            let score_db = daily::DailyUserScore::new();
            if let Err(e) = score_db.update_score(
                uid,
                &quiz_state.subject,
                &quiz_state.grade,
                result.is_correct,
            ...
            // 记录答题历史
            let record_db = daily::DailyAnswerRecord::new();
            let _ = record_db.insert(
                uid,
                &quiz_state.idsubject,
                &quiz_state.question_id,
                &quiz_state.question,
                user_answer,
                &quiz_state.standard_answer,
                result.is_correct,
